...
void Json::jsonLdrPot(const std::string& filename, Client &client){
    ...
    // if (!fileJson.IsNull()) {
    //     // Validate JSON structure
    //     if (!fileJson.IsObject() || !fileJson.HasMember("ldr") || !fileJson.HasMember("pot")) {
    //         std::cerr << "Invalid JSON structure in file" << std::endl;
    //         return;
    //     }
        ...
        // Now you can use fileStrip and fileRgb in your code
        std::cout << "Read strip from file: " << fileLdr << std::endl;
        std::cout << "Read rgb from file: " << filePot << std::endl;
